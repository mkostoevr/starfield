...
class Starfield {
public:
	Starfield(int count, float spread, float speed)
	: m_spread(spread)
	, m_speed(speed)
	, m_starsX(count)
	, m_starsY(count)
	...
	void update_and_render(Backend &backend, float delta)
	{
...
template<class Backend>
static void render(Starfield &starfield, Backend &backend, size_t delta_ns)
{
...
#ifdef _WIN32
#include "main_windows.cc"
#include "Backend_Win32_Gdi.cc"
#else
